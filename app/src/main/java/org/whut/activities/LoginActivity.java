...
import org.whut.utils.AccountInfoManager;
...
public class LoginActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		 ...
		 * 测试用户，实际使用请时删除
		 *****************************/
		User user = new User("root",MD5Utils.string2MD5("root"));
		dao.addUser(user);
			...
			public void handleMessage(Message msg) {
				...
				case 2:
					...
					if(msg.obj!=null&&(msg.obj.toString().equals("no_internet"))){
						...
						User mUser = new User();
						AccountInfoManager account = AccountInfoManager.getmInstance();
						account.setmUser(mUser);
					}else{
						//有网模式登录成功
						userName = edt_username.getText().toString();
						//将用户名、密码存入本地数据库
		...
		public void run() {
			...
			if(TextUtils.isEmpty(edt_username.getText().toString())||TextUtils.isEmpty(edt_password.getText().toString())){
				...
				try{
					if(MODE_TAG){
						//有网模式
						boolean result = MyClient.getInstance().login(edt_username.getText().toString(), edt_password.getText().toString(), UrlStrings.SESSIONID_URL);
							Log.i("msg", result+"");
						if(!result){
								//登录失败
								msg.what = 1;
